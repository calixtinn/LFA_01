...
\begin{document} 
	...
	\begin{itemize}
		...
		\item \textbf{Equivalência de AFD's:} A função de equivalência de AFD's utilizou de outras funções antes implementadas no trabalho. Dois autômatos são equivalentes, se as propriedades abaixo são satisfeitas:
		
		\begin{enumerate}
			\item O alfabeto deve ser igual
			\item A quantidade de estados, após aplicada a minimização de ambos, deve ser igual
			\item Os estados inicais dos dois AFD's devem ser equivalentes.
		\end{enumerate}
